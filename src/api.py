...
MANIFEST_FILES = [
    ...
    "ui/tests/global-setup.ts",
    "ui/tests/global-teardown.ts",
    "ui/playwright.config.ts",
    # ONLY the default profile — canonical schema template provided by Claude.
    # User profiles (Experiment-*, custom profiles) are user data — NOT tracked here.
    # __system__* and __testui__ are generated by --reset-system-profiles — NOT tracked.
    "profiles/default/person.json",
    "profiles/default/withdrawal_schedule.json",
    "profiles/default/income.json",
    "profiles/default/allocation_yearly.json",
    "profiles/default/inflation_yearly.json",
    "profiles/default/shocks_yearly.json",
    "profiles/default/economic.json",
]

# Guard: ensure no non-default profile paths ever sneak in
assert all(
    not f.startswith("profiles/") or f.startswith("profiles/default/")
    for f in MANIFEST_FILES
), "MANIFEST_FILES must only include profiles/default/ — user profiles are not tracked"
...
def on_startup() -> None:
    """
    On server start:
      1. Verify all tracked files against manifest.lock pinned hashes — EXIT if mismatch.
      2. Build runtime manifest (served at /manifest for --checkupdates).
      3. Auto-snapshot all user-managed profiles.
    """
    global _STARTUP_MANIFEST
    import sys as _sys, json as _json2, hashlib as _hl2

    lock_path = os.path.join(APP_ROOT, "manifest.lock")
    if os.path.isfile(lock_path):
        try:
            with open(lock_path) as _lf:
                _lock = _json2.load(_lf)

            # Verify _self_crc — detects manifest.lock corruption
            _tracked = _lock.get("tracked", [])
            _expected_crc = _lock.get("_self_crc", "")
            _actual_crc   = _hl2.sha256(
                _json2.dumps(_tracked, sort_keys=True).encode()
            ).hexdigest()[:16]
            if _expected_crc and _actual_crc != _expected_crc:
                print("[FATAL] manifest.lock _self_crc mismatch — file is corrupted.")
                print(f"  expected: {_expected_crc}  actual: {_actual_crc}")
                raise RuntimeError("manifest.lock corrupted — cannot start server safely")

            _pinned = _lock.get("hashes", {})
            if not _pinned:
                print("[startup] manifest.lock: no pinned hashes (first-time setup). Starting.")
            else:
                _mismatches = []
                _verified   = 0
                for _fname, _pinned_hash in _pinned.items():
                    # Skip non-default profile entries — user profiles are not tracked
                    if _fname.startswith("profiles/") and not _fname.startswith("profiles/default/"):
                        continue
                    _path    = os.path.join(APP_ROOT, _fname)
                    _current = _file_sha256(_path)
                    if _current != _pinned_hash:
                        _mismatches.append((_fname, _pinned_hash, _current))
                    else:
                        _verified += 1

                if _mismatches:
                    print("[FATAL] manifest.lock verification failed — deployed files don't match:")
                    for _fn, _ph, _ch in _mismatches:
                        print(f"  {_fn}")
                        print(f"    expected: {_ph}")
                        print(f"    found:    {_ch}")
                        if _ch == "missing":
                            print(f"    → FILE MISSING — copy from latest Claude session")
                        else:
                            print(f"    → STALE — copy latest version from Claude session")
                    print("\n  Copy the correct files and restart. No other steps required.")
                    raise RuntimeError(
                        f"manifest.lock mismatch on {len(_mismatches)} file(s) — "
                        "server refuses to start with stale/wrong code"
                    )
                print(f"[startup] manifest.lock: {_verified}/{len(_pinned)} pinned files verified ✅")

        except RuntimeError:
            raise
        except Exception as _me:
            print(f"[startup] WARNING: manifest.lock check failed unexpectedly: {_me}")
    else:
        print("[startup] WARNING: manifest.lock not found — skipping integrity check.")

    # Build runtime manifest for /manifest endpoint
    _STARTUP_MANIFEST = _build_manifest()
    print("[startup] Source file manifest (runtime hashes):")
    for name, info in _STARTUP_MANIFEST["files"].items():
        status = info["sha256_short"] if info["exists"] else "⚠ MISSING"
        print(f"  {name:<40} {status}")

    # Auto-snapshot all user-managed profiles (skip __*, default, dot-dirs)
...
def list_profiles():
    """
    Returns user-managed profiles visible in the UI dropdown.
    Filters out __* (system fixtures managed by test_flags.py) and .* (OS artifacts).
    All filtered profiles remain accessible via /run, /profile-config etc.
    """
    _ensure_dir(PROFILES_ROOT)
    names = []
    for d in os.listdir(PROFILES_ROOT):
        if d.startswith(("__", ".")):   # hidden: system fixtures and dot-dirs
            continue
...
def _apply_ira_contribution_rules(
    ...
    """
    Silently enforce IRS IRA/Roth contribution rules on deposits_yearly.
    Modifies alloc_accounts["deposits_yearly"] in-place.

    Rules enforced (2025/2026 IRS limits):
      1. Earned income requirement: W2 > 0 required. Rental/SS/dividends do NOT qualify.
      2. Annual limit per account: min(W2, $7,000 under-50 | $8,000 age 50+).
      3. Roth IRA income phase-out: MFJ $236K-$246K, Single $150K-$165K.
         MAGI approximated as W2 + rental + ordinary_other.
      4. TRAD IRA: always allowed regardless of income level (no phase-out on contribution).

    All enforcement is silent — deposits are clamped/zeroed with a log message only.
    """
    _log = logging.getLogger(__name__)
    IRA_LIMIT_UNDER_50 = 7_000.0
    IRA_LIMIT_50_PLUS  = 8_000.0
    ROTH_PO_MFJ_LOW,  ROTH_PO_MFJ_HIGH  = 236_000.0, 246_000.0
    ROTH_PO_SNG_LOW,  ROTH_PO_SNG_HIGH  = 150_000.0, 165_000.0
    is_mfj = str(filing).upper() in ("MFJ", "MFS")

    acct_types   = alloc_accounts.get("account_types", {}) or {}
    deposits_ref = alloc_accounts.get("deposits_yearly", {}) or {}
    w2_arr   = np.asarray(income_cfg.get("w2",             [0.0] * n_years), dtype=float)
    rent_arr = np.asarray(income_cfg.get("rental",         [0.0] * n_years), dtype=float)
    ord_arr  = np.asarray(income_cfg.get("ordinary_other", [0.0] * n_years), dtype=float)

    for acct, acct_type in acct_types.items():
        if acct_type not in ("traditional_ira", "roth_ira"):
            continue
        if acct not in deposits_ref:
            continue
        dep = deposits_ref[acct].copy()
        changed = False

        for y in range(min(n_years, len(dep))):
            if dep[y] <= 0.0:
                continue

            age_y  = current_age + y + 1
            w2_y   = float(w2_arr[y])   if y < len(w2_arr)   else 0.0
            orig   = float(dep[y])

            # Rule 1: earned income required
            if w2_y <= 0.0:
                dep[y] = 0.0
                changed = True
                _log.info("[IRA rules] %s yr%d (age %d): zeroed $%,.0f — no W2 earned income",
                          acct, y + 1, age_y, orig)
                continue

            # Rule 2: age-based annual limit
            limit   = IRA_LIMIT_50_PLUS if age_y >= 50 else IRA_LIMIT_UNDER_50
            allowed = min(w2_y, limit)

            # Rule 3: Roth income phase-out
            if acct_type == "roth_ira":
                magi_y  = (w2_y
                           + (float(rent_arr[y]) if y < len(rent_arr) else 0.0)
                           + (float(ord_arr[y])  if y < len(ord_arr)  else 0.0))
                po_low  = ROTH_PO_MFJ_LOW  if is_mfj else ROTH_PO_SNG_LOW
                po_high = ROTH_PO_MFJ_HIGH if is_mfj else ROTH_PO_SNG_HIGH
                if magi_y >= po_high:
                    allowed = 0.0
                    _log.info("[IRA rules] %s yr%d (age %d): zeroed $%,.0f Roth — "
                              "MAGI $%,.0f above phase-out $%,.0f",
                              acct, y + 1, age_y, orig, magi_y, po_high)
                elif magi_y > po_low:
                    po_frac = (magi_y - po_low) / (po_high - po_low)
                    allowed = min(allowed, round(limit * (1.0 - po_frac)))
                    _log.info("[IRA rules] %s yr%d (age %d): Roth reduced $%,.0f→$%,.0f "
                              "(MAGI $%,.0f in phase-out)",
                              acct, y + 1, age_y, orig, allowed, magi_y)

            if dep[y] > allowed + 0.01:
                if allowed > 0:
                    _log.info("[IRA rules] %s yr%d (age %d): clamped $%,.0f→$%,.0f "
                              "(limit $%,.0f, W2 $%,.0f)",
                              acct, y + 1, age_y, orig, allowed, limit, w2_y)
                dep[y] = allowed
                changed = True

        if changed:
            alloc_accounts["deposits_yearly"][acct] = dep
...
def run_simulation(payload: Dict[str, Any] = Body(...)):
    ...
    # Inject Additional Medicare Tax (AMT) thresholds into tax_cfg.
    # IRC §3101(b)(2): 0.9% on W2 wages above $200K (single) / $250K (MFJ).
    # taxes_core.py reads ADDL_MEDICARE_THRESH and ADDL_MEDICARE_RATE from tax_cfg.
    # These are not in the tax JSON files — injected here based on filing status.
    if tax_cfg is not None:
        _amt_thresh = 250_000.0 if filing.upper() in ("MFJ", "MFS") else 200_000.0
        tax_cfg["ADDL_MEDICARE_THRESH"] = _amt_thresh
        tax_cfg["ADDL_MEDICARE_RATE"]   = 0.009
    person_cfg = load_person(person_path)
    ...
    # Enforce IRA/Roth contribution rules on deposits_yearly
    _apply_ira_contribution_rules(
    ...
    if modular_test:
        ...
        if _ss_cfg and not _ss_cfg.get("exclude_from_plan", False):
            ...
            for _y in range(_ny):
                ...
                # ── Social Security inclusion: three-tier provisional income rule ──────
                ...
                #
                # NOTE: Provisional income here uses only income.json entries (W2, rental,
                # interest). Withdrawals and RMDs are not yet computed at this stage — the
                # simulator handles those. For RMD-era retirees with significant TRAD IRA
                # distributions, actual inclusion will almost always reach 85%.
                # Provisional income thresholds (not inflation-adjusted by statute):
                if _filing == "MFJ":
                    _pi_base1, _pi_base2 = 32_000.0, 44_000.0
                else:
                    _pi_base1, _pi_base2 = 25_000.0, 34_000.0

                # Non-SS ordinary income available at this stage (income.json sources only)
                _w2_arr      = np.asarray(income_cfg.get("w2", [0.0] * _ny), dtype=float)
                _rental_arr  = np.asarray(income_cfg.get("rental", [0.0] * _ny), dtype=float)
                _interest_arr = np.asarray(income_cfg.get("interest", [0.0] * _ny), dtype=float)
                _non_ss_y = 0.0
                if _y < len(_w2_arr):      _non_ss_y += float(_w2_arr[_y])
                if _y < len(_rental_arr):  _non_ss_y += float(_rental_arr[_y])
                if _y < len(_interest_arr): _non_ss_y += float(_interest_arr[_y])

                _provisional = _non_ss_y + 0.5 * _ss_gross

                if _provisional < _pi_base1:
                    _ss_taxable = 0.0
                elif _provisional < _pi_base2:
                    _ss_taxable = min(0.5 * _ss_gross, 0.5 * (_provisional - _pi_base1))
                else:
                    # 85% of (provisional - base2) + 50% of (base2 - base1), capped at 85% of SS
                    _ss_taxable = min(
                        0.85 * _ss_gross,
                        0.85 * (_provisional - _pi_base2) + 0.5 * (_pi_base2 - _pi_base1)
                    )

                _ss_by_year.append(_ss_taxable)

            # Inject into income_cfg as ordinary_other.
            # IMPORTANT: Only override years where ordinary_other is currently 0.
            # If income.json already has non-zero ordinary_other for SS-active years
            # (e.g. user manually entered SS as ordinary income), preserve that value.
            # The SS block is used to compute injection for years where the user
            # hasn't manually specified ordinary_other.
            if "ordinary_other" in income_cfg and hasattr(income_cfg["ordinary_other"], "__len__"):
                ...
                for _y, _ss in enumerate(_ss_by_year):
                    if _y < len(_arr) and _arr[_y] == 0.0:   # preserve manual entries
                        _arr[_y] = _ss
        ...
        # misc income: taxable portion added to ordinary income for tax engine
        # non-taxable portion (gifts/inheritances) still contributes to surplus routing
        _misc_arr     = np.asarray(income_cfg.get("misc",          [0.0] * _n_years), dtype=float)
        _misc_tax_arr = np.asarray(income_cfg.get("misc_taxable",  [1.0] * _n_years), dtype=float)
        if _misc_arr.size < _n_years:
            _misc_arr     = np.concatenate([_misc_arr,     np.zeros(_n_years - _misc_arr.size)])
            _misc_tax_arr = np.concatenate([_misc_tax_arr, np.ones(_n_years - _misc_tax_arr.size)])
        ...
        # W2 wages per path per year — for Additional Medicare Tax (0.9%)
        w2_income_cur_paths       = np.zeros((paths, _n_years), dtype=float)
        # Income.json sources only (pre-RMD/conversion) — for portfolio withdrawal offset
        # Includes ALL income sources (gross) — excess_income_policy applies tax approximation
        ...
        _excess_income_policy = (econ_policy.get("defaults", {}) or {}).get("excess_income_policy", {}) if econ_policy else {}
        ...
        res = run_accounts_new(
            ...
            w2_income_cur_paths=w2_income_cur_paths,
            income_sources_cur_paths=income_sources_cur_paths,
            excess_income_policy=_excess_income_policy,
            person_cfg=person_cfg,
    ...
    # 9a-income) Inject income.json data into person_cfg for the Roth optimizer.
    # loaders.py pre-processes ages/years format into flat year-indexed arrays (0-based,
    # index 0 = year 1 = current_age+1). This block runs after build_income_streams
    # and after SS injection, so ordinary_other reflects the final income stream.
    try:
        _ic = income_cfg if isinstance(income_cfg, dict) else {}

        def _flat_yr1(arr):
            """Year-1 (index 0) value from a pre-processed flat income array."""
            if not arr or len(arr) == 0:
                return 0.0
            v = arr[0]
            # Guard: if loaders didn't pre-process, arr[0] might still be a dict
            return float(v) if not hasattr(v, "get") else 0.0

        _w2_flat   = list(_ic.get("w2",            []) or [])
        _ord_flat  = list(_ic.get("ordinary_other", []) or [])
        _rent_flat = list(_ic.get("rental",         []) or [])

        # Retirement income: first non-zero ordinary_other value in post-retirement years.
        # This drives future_rmd_rate in the optimizer (non-RMD income at RMD age).
        _ret_yr_offset = max(0, int(round(float(person_cfg.get("retirement_age", 65)) - _current_age)))
        _retirement_income = 0.0
        for _v in (_ord_flat[_ret_yr_offset:] if len(_ord_flat) > _ret_yr_offset else []):
            if not hasattr(_v, "get") and float(_v) > 0:
                _retirement_income = float(_v)
                break

        person_cfg["income_data"] = {
            "w2_yr1":           _flat_yr1(_w2_flat),
            "rental_yr1":       _flat_yr1(_rent_flat),
            "ordinary_yr1":     _flat_yr1(_ord_flat),
            # Full arrays so _income_at_age() uses exact per-year values in the schedule
            "w2_by_year":       _w2_flat,
            "ordinary_by_year": _ord_flat,
            # Retirement income drives future RMD rate computation in optimizer
