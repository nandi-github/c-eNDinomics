...
MANIFEST_FILES = [
    ...
    # Default profile — canonical schema reference provided by Claude.
...
def on_startup() -> None:
    ...
    # ── Step 1: manifest.lock integrity verification ─────────────────────────
    # manifest.lock is committed to git with pinned SHA256 hashes of all tracked
    # source files. If ANY deployed file doesn't match its pinned hash, the server
    # refuses to start — this catches missed file copies and corrupted deployments.
    #
    # Workflow after Claude provides new files:
    #   1. Copy files to disk
    #   2. python3 -B test_flags.py --reset-manifest   ← recomputes + pins new hashes
    #   3. ./vcleanbld_ui                               ← server now starts clean
    import sys as _sys
    ...
    if os.path.isfile(lock_path):
        try:
            ...
            if _expected_crc and _actual_crc != _expected_crc:
                ...
                print(f"  expected: {_expected_crc}")
                print(f"  actual:   {_actual_crc}")
                print("  Run: python3 -B test_flags.py --reset-manifest")
                raise RuntimeError("manifest.lock corrupted — cannot start server safely")

            _pinned = _lock.get("hashes", {})
            if not _pinned:
                # No hashes yet — first-time setup. Allow startup, no action needed.
                # Claude ships manifest.lock with hashes pre-computed for the files
                # Claude provided. If hashes is empty, all files are unverified.
                print("[startup] manifest.lock: no pinned hashes (first-time setup). Starting.")
            else:
                # Iterate over the hashes dict — only verify files that have pinned hashes.
                # Files in tracked but NOT in hashes are silently skipped (not yet pinned).
                # Claude ships manifest.lock with hashes for the specific files Claude
                # provided this session. No manual step required from the user.
                _mismatches = []
                _verified   = 0
                ...
                if _mismatches:
                    ...
                    print()
                    print("  Copy the correct files and restart. No other steps required.")
                    raise RuntimeError(
            ...
            raise  # re-raise — uvicorn will exit
        except Exception as _me:
            print(f"[startup] WARNING: manifest.lock check failed unexpectedly: {_me}")
            print("[startup] Proceeding without manifest verification.")
    else:
        print("[startup] WARNING: manifest.lock not found — file integrity cannot be verified.")
        print("[startup] Copy manifest.lock from the latest Claude session alongside other files.")

    # ── Step 2: Build runtime manifest for /manifest endpoint ────────────────
    ...
    # ── Step 3: Auto-snapshot all user-managed profiles ───────────────────────
    # Skips: default (non-editable), __* (system fixtures), .* (OS artifacts)
...
def list_profiles():
    ...
    Filters out:
      __*  — system fixture profiles (hidden, managed by test_flags.py --reset-system-profiles)
      .*   — dot-directories (OS artifacts)
    The 'default' profile IS included as a visible read-only template.
    All filtered profiles remain accessible via /run, /artifact, /profile-config etc.
    """
    _ensure_dir(PROFILES_ROOT)
    names = []
